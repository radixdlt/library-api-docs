...
REPO="https://github.com/radixdlt/radixdlt-js.git"
...
for tag in $TAGS; do
    ...
    # Run yarn to get the tsdoc.  Note that we ignore errors here as our
    # javadoc is a bit crusty at the moment.
    ( cd tmp && yarn install && yarn doc:html || /usr/bin/true )

    # Remove old stuff, and copy new stuff over
    rm -rf "radixdlt-js/$tag"
    mkdir -p "radixdlt-js/$tag"
    cp -a "$TMP/build/docs/" "radixdlt-js/$tag/"
    rm -rf "$TMP"
done
